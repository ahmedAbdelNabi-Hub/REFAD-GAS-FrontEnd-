...
export class AddCarComponent implements OnInit {
  ...
  userCompanyId = this.auth.UserId;
  ...
  private setupCompanyId() {
    debugger;
    if (!this.isAdmin()) {
      const companyId = this.userCompanyId();
      if (companyId) {
        this.carForm.patchValue({ companyId });
      }
    }
  }
  ...
  onSubmit() {
    console.log('Submitting form:', this.carForm.value);
    ...
    this.carService.createCar(formData).subscribe({
      next: (res: IBaseApiResponse<number>) => {
        ...
        if (res.statusCode === 200) this.router.navigate(['/dashboard/cars']);
